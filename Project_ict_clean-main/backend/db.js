...
if (!supabaseUrl || !supabaseKey) {
    console.error('❌ ไม่พบ SUPABASE_URL หรือ SUPABASE_SERVICE_KEY ในไฟล์ .env');
    process.exit(1);
}

const supabase = createClient(supabaseUrl, supabaseKey);

console.log('✅ กำหนดค่าเชื่อมต่อ Supabase SDK สำเร็จ!');
...
async function initDatabase() {
    try {
        ...
        console.error('⚠️ ข้อผิดพลาดในการเตรียมข้อมูล:', err.message);
    }
}

module.exports = { supabase, initDatabase, UP_ICT_FACULTY };
