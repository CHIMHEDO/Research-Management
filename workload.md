...
- [x] **5. ระบบยืนยันสัดส่วนผู้ร่วมงานและกฎอนุมัติอัตโนมัติ 7 วัน (Co-author Confirmation & 7-Day Auto-Lock Rule)**:
  - **บันทึกผลงาน (Submit & Status `PENDING`):** เมื่ออาจารย์คนที่ 1 บันทึกผลงานพร้อมระบุสัดส่วนผู้แต่งหลายคน ระบบจะบันทึกสถานะเป็น `PENDING` และกำหนดเส้นตายล่วงหน้า 7 วัน (`deadline = now + 7 days`)
  - **แจ้งเตือนผู้ร่วมงานอัตโนมัติ (Email Trigger):** Backend ค้นหาอีเมลของผู้ร่วมงานในฐานข้อมูลและส่งอีเมลแจ้งเตือนสัดส่วนพร้อมปุ่มกดยืนยันเข้าสู่อีเมลทันที
  - **ปุ่มกดยืนยันสัดส่วน (Confirm Proportion Button):** อาจารย์ผู้ร่วมงานที่ล็อกอินเข้ามาจะเห็นปุ่ม "ยืนยันสัดส่วน" บนการ์ดผลงานและตาราง เมื่อกดยืนยันครบทุกคน สถานะจะเปลี่ยนเป็น `CONFIRMED`
  - **กฎล็อคอัตโนมัติ 7 วัน (7-Day Auto-Lock Rule):** หากพ้นกำหนด 7 วันโดยไม่มีการคัดค้านหรือแก้ไขสัดส่วน ระบบจะปรับสถานะเป็น `AUTO_CONFIRMED` ให้อัตโนมัติ
  - **เกณฑ์การนับภาระงานสะสม (Workload Accumulation Gate):** ผลงานที่มีสถานะ `PENDING` จะยังไม่ถูกนำไปนับรวมในยอดชั่วโมงภาระงานสะสม (Totals / KPIs) จนกว่าจะได้รับการยืนยันครบถ้วน (`CONFIRMED` หรือ `AUTO_CONFIRMED`)
...
timeline
    ...
    Phase 3 : ระบบตรวจจับงานซ้ำ (Duplicate & Conflict Detector) : เครื่องมือตรวจสอบสัดส่วนผู้ร่วมงานรวม 100% : หน้าจัดการภาพรวมอาจารย์ทั้งคณะ
    Phase 4 : โมดูลบันทึกการเบิกจ่ายเงิน (Disbursals) : ตาราง Audit Trail Logs บันทึกประวัติการแก้ไข
    Phase 5 : ระบบยืนยันสัดส่วนผู้ร่วมงาน (Co-author Confirmation) : กฎอนุมัติอัตโนมัติ 7 วัน (7-Day Auto-Lock) : อีเมลแจ้งเตือนผู้แต่งอัตโนมัติ
```

---

### ✅ รายละเอียดผลการทดสอบระบบล่าสุด:
- [x] **Frontend Validation & Build**: ผ่าน 100% ปลอดภัย ไม่มี ReferenceError / White Screen
- [x] **Backend Endpoint**: `POST /api/entries/:id/confirm` ตรวจสอบสิทธิ์ผู้ยืนยันและอัปเดตสถานะครบถ้วน
- [x] **Workload Counting Protection**: ชั่วโมงภาระงานสะสมจะคำนวณเฉพาะผลงานที่ยืนยันแล้วเท่านั้น
- [x] **Edit Preservation**: ฟังก์ชันปรับแต่ง (Edit) โหลดและบันทึกรายชื่อผู้แต่งทุกคนครบถ้วน ไม่ทำให้สถานะ `PENDING` หลุดหาย